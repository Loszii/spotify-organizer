...
def getArtistDate(current, prev):
    if prev == "":
        return current
    else:
        return prev
...
def organize(data):
    for j in data:
        for i in j:
            allSongs[(i["trackName"], i["artistName"])] = [allSongs[(i["trackName"], i["artistName"])][0] + i["msPlayed"], getSongDate(i)]
...
def sortByArtist(config):
    ...
    for i in allSongs:
        dataPerArtist[i[1]] = [0, ""]
    for i in allSongs:
        dataPerArtist[i[1]] = [dataPerArtist[i[1]][0] + allSongs[i][0], getArtistDate(allSongs[i][1], dataPerArtist[i[1]][1])]
    if config == "d":
        for i in dataPerArtist:
            if dataPerArtist[i][0] >= mins:
                final.append((dataPerArtist[i][1], i, round(dataPerArtist[i][0], 1), round(dataPerArtist[i][0] / 60, 1))) #need to round again since floating points
                #final = [(date, artist, min, hour)...]
        ...
        for i in dataPerArtist:
            if dataPerArtist[i][0] >= mins: # 
                final.append((round(dataPerArtist[i][0], 1), i, dataPerArtist[i][1], round(dataPerArtist[i][0] / 60, 1))) #need to round again since floating points
                #final = [(min, artist, date, hour)...]
        allArtists = getTotalArtist()
        final.sort()
        txt = open("output.txt", "a", encoding="utf-8")
        txt.write("-------------------------------------------------\n" * 5)
